...
This framing is useful because scaling relations are not just empirical lines on a log-log plot. They are the bridge between a quantity we can observe efficiently, the X-ray emission from hot gas, and the underlying quantity that matters for cosmology, the halo mass. At the same time, departures from a perfect relation are scientifically meaningful: the scatter is where cooling, feedback, mergers, projection, and background systematics show up.
...
In presentation language, \Tx--\Mfive{} should be introduced as the closer-to-gravity relation: it asks whether the gas temperature remembers the halo potential. \Lx--\Mfive{} is the more astrophysically complicated relation: it asks whether the gas density structure and emissivity still preserve a usable mass trend after cluster-specific baryonic processes have acted. The \Lx--\Tx{} relation then acts as an internal X-ray consistency check, connecting the two gas observables without using the lensing mass on either axis.
...
The practical logic for the talk is therefore: first show the self-similar expectation as the gravity-only reference, then explain why the measured relation can be shallower, steeper, or broader. Core excision is one diagnostic in that logic. If the cluster center dominates the luminosity through a cool core, removing the inner $0.15\Rfive$ should change \Lx{} and possibly reduce scatter. If the result is stable to core excision, then the headline mass trend is less likely to be driven only by central gas physics.
...
For a 20-minute report, this also provides a clean slide-to-slide transition. The weak-lensing catalog supplies the independent mass axis; Chandra supplies the X-ray spectra; Sherpa converts those spectra into \Tx{} and \Lx{}; and \texttt{linmix} turns the measured points into slopes, normalizations, and intrinsic scatter. The rest of the report follows that same order.
...
\end{align}
The fixed exponents match the self-similar comparison convention. The chosen pivots define the reported normalization rather than the slope: moving the pivot would transform the intercept but should not change the physical line, intrinsic scatter, or slope if the fit is otherwise unchanged. Because the retained clusters are mostly above $3\times10^{14}M_\odot$, a future normalization-focused analysis should also quote a sample-centered pivot, but the present report focuses on slopes and scatter. \Mfive{} errors are taken from the literature mass references encoded in the canonical products. \Rfive{} uncertainties are propagated from \Mfive{} as aperture provenance and are not added as independent regression errors.

\section{Results}

\subsection{Full-\texorpdfstring{$R_{500}$}{R500} Baseline}

The main full-\Rfive{} scaling sample contains 18 clusters. Figure~\ref{fig:full-scaling} shows the three baseline relations. The luminosity--mass fit gives $\beta=1.08^{+0.53}_{-0.49}$ and intrinsic scatter 0.169 dex. This is statistically consistent with the self-similar slope of $4/3$, but the uncertainty is large enough that it does not strongly discriminate among literature values. The temperature--mass fit gives $\beta=0.48^{+0.26}_{-0.25}$ and intrinsic scatter 0.117 dex, slightly shallower than but consistent with the self-similar value of $2/3$. The luminosity--temperature relation gives $\beta=0.76^{+0.43}_{-0.41}$ and intrinsic scatter 0.227 dex; it is the noisiest relation and is shallower than common Chandra literature comparisons such as \citet{maughan2012}.
...
The core-excised branch measures the same retained 18 clusters over $0.15$--$1.0\Rfive$. Figure~\ref{fig:core-scaling} shows the corresponding fits. The core-excised \Lx--\Mfive{} slope is $\beta=1.16^{+0.53}_{-0.49}$ with scatter 0.192 dex, and the \Tx--\Mfive{} slope is $\beta=0.56^{+0.30}_{-0.31}$ with scatter 0.134 dex. The \Lx--\Tx{} slope is $\beta=0.90^{+0.46}_{-0.43}$ with scatter 0.230 dex. These values agree with the full-\Rfive{} mass-based slopes within the statistical uncertainties. In this small heterogeneous sample, core excision does not reduce the luminosity--mass scatter; the likely reasons are sample size, retained high-temperature systems, heterogeneous Chandra coverage, and residual sensitivity to background and aperture choices.
...
Aperture & Relation & $N$ & $\beta$ & Intrinsic scatter & Note \\
 & & & & (dex) & \\
\midrule
Full \Rfive{} & \Lx--\Mfive{} & 18 & $1.08^{+0.53}_{-0.49}$ & 0.169 & Baseline \\
Full \Rfive{} & \Tx--\Mfive{} & 18 & $0.48^{+0.26}_{-0.25}$ & 0.117 & Baseline \\
Full \Rfive{} & \Lx--\Tx{} & 18 & $0.76^{+0.43}_{-0.41}$ & 0.227 & Noisiest relation \\
$0.15$--$1.0\Rfive$ & \Lx--\Mfive{} & 18 & $1.16^{+0.53}_{-0.49}$ & 0.192 & Core-excised \\
$0.15$--$1.0\Rfive$ & \Tx--\Mfive{} & 18 & $0.56^{+0.30}_{-0.31}$ & 0.134 & Native core \Tx{} intervals \\
$0.15$--$1.0\Rfive$ & \Lx--\Tx{} & 18 & $0.90^{+0.46}_{-0.43}$ & 0.230 & Native core \Tx{} intervals \\
\bottomrule
\end{tabular}
\end{table}
...
The full-\Rfive{} and core-excised mass-based slopes agree within the current uncertainties. This stability is encouraging: it suggests that the main mass calibration is not driven purely by cool-core emission, despite the heterogeneous sample. At the same time, the luminosity--mass scatter does not improve after core excision, unlike in some larger and more controlled literature samples \citep{pratt2009,mantz2010,mantz2016}. This difference should not be over-interpreted. The present project has only 18 clusters in the baseline fit, a mix of CLASH and LoCuSS selection, non-uniform Chandra depth, and several high-temperature retained systems.

Compared with the literature, the \Tx--\Mfive{} slope is broadly compatible with self-similar expectations and with the idea that temperature is a relatively low-scatter mass proxy. The \Lx--\Mfive{} slope is lower than many steep luminosity--mass measurements but is consistent with self-similar within the large uncertainty. The shallow \Lx--\Tx{} slope is the least robust result and likely reflects the combination of temperature outliers, small dynamic range, and measurement covariance. A publication-level version should strengthen the background QA, audit any remaining fallback intervals in excluded objects, and eventually use a hierarchical model with explicit selection effects if the sample definition is expanded.
...
\item The full-\Rfive{} baseline gives $\beta_{\Lx-M}=1.08^{+0.53}_{-0.49}$ and $\beta_{\Tx-M}=0.48^{+0.26}_{-0.25}$, with intrinsic scatter 0.169 dex and 0.117 dex.
