...
# the catalog key. Add new processed cluster folders here as they are created.
CLUSTER_DATA_DIRS = {
...
def write_aperture_overlay_plot(
    ...
    if not show_background_annulus:
        ax.text(
            0.02,
            0.075,
            "background: CIAO blank-sky events",
            transform=ax.transAxes,
            color="white",
            fontsize=8,
            bbox={"facecolor": "black", "alpha": 0.55, "edgecolor": "none"},
...
if background_mode == "wstat":
    # WSTAT is Sherpa's Poisson source+background likelihood for PHA data.
    # It uses the background PHA statistically and does not subtract counts.
    set_stat("wstat")
else:
    # Legacy comparison mode: subtract background and fit with chi-square.
...
residual_summaries = []
plot_caveat = (
    "CIAO/Sherpa WSTAT caveat: this plot is qualitative only; Sherpa data/model/residual plots do not include the background component used internally by WSTAT."
    if background_mode == "wstat"
    else "Background-subtracted chi-square diagnostic plot."
)
if background_mode == "wstat":
    print("[warn]", plot_caveat)
...
if background_mode == "wstat":
    ax.text(
        0.02,
        0.03,
        "WSTAT fit: plot/residuals are qualitative only",
        transform=ax.transAxes,
        fontsize=8,
        color="0.2",
...
vals = {{
    ...
    "diagnostic_checks": {{
        ...
        "visual_check": "for WSTAT the plot is qualitative only because Sherpa plot_data/plot_fit/plot_resid do not include the background component; for subtract mode inspect data/model agreement and residuals",
        "ciao_wstat_rule": "do not call subtract() with WSTAT; background PHA is automatically included in the statistic",
        "ciao_plot_caveat": plot_caveat,
...
def main() -> None:
    ...
    write_sherpa_script(sherpa_script, spectrum_pis, redshift, nh, fit_json, fit_plot, args.sherpa_background_mode)
